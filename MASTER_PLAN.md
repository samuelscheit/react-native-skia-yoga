...
- Monitor active worker 069 example native-generation local-artifact preservation hardening, then accept or reject it based on its tmux goal gate, implementation/report scope, sentinel preservation evidence, Node-run Expo CNG evidence, generated project/autolinking assertions, cleanup guarantees, proof-boundary wording, nested challenger documentation, full verification matrix, and cleanup state.
