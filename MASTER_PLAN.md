...
- Highest-value next target remains actual React Native runtime integration
  proof for dynamic layout/style delivery through a real app bridge, but local
  platform tooling blockers still apply.
- If continuing before platform tooling changes, schedule an audit-only worker
  to find a fresh evidence-backed gap rather than extending generated/native
  layout breadth speculatively.
