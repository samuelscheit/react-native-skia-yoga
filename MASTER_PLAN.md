...
Latest accepted implementation: worker 169 added whole `style.matrix`
`SharedValue` public/Reconciler proof and nested matrix `SharedValue<number>`
rejection coverage.
...
- Launch Worker 170 for a post-Worker 169 root-cause audit from an isolated
  worktree and branch.
- Expected Worker 170 scope: accept or reject Worker 169's proof boundary,
  reconfirm the latest focused/post-merge evidence and local platform blockers,
