...
Latest integration: worker 146 accepted the post-worker-145 proof surface,
reconfirmed the 28-command feasible matrix, and selected
`YogaNode::setStyle(...)` SkPaint-backed `backgroundColor` ordering for
explicit style paint fields as the next strongest locally unblocked target.
